...
exports.RestServer = function () {
	...
	/*Query*/
	server.get('/api/:table/:query?', function (req, res) {
		res.json({message: 'Get items in ' + req.params.table});
	});

	/* Create */
	server.post('/api/:table', function (req, res) {
		res.json({message: 'Create item ' + JSON.stringify(req.body) + ' in ' + req.params.table});
	});

	/* Read */
	server.get('/api/:table/:id?', function (req, res) {
		res.json({message: 'Get item ' + req.params.id + ' in ' + req.params.table});
	});
